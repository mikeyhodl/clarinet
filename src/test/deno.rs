    ...
    let join_handles = test_modules.iter().map(move |main_module| {
        ...
        tokio::task::spawn_blocking(move || {
            let join_handle = std::thread::spawn(move || {
                let future =
                    run_test_file(program_state, main_module, test_module, permissions, sender);
...
fn setup_chain(state: &mut OpState, args: Value, _: ()) -> Result<String, AnyError> {
    ...
    let (session_id, accounts, contracts) =
        sessions::handle_setup_chain(args.name, args.transactions)?;
    let serialized_contracts = contracts
        .iter()
        .map(|(a, s)| {
            json!({
